...
**`.claude/CAP_NHAT_AI.md`** — file riêng theo dõi thông tin mới nhất về chính model Claude/Anthropic
(để agent phiên sau không lạc hậu so với training cutoff). Đọc file đó nếu cần biết model/tính năng
Claude mới nhất; cập nhật lại nó ngay khi phát hiện thông tin mới hơn (xem quy tắc cập nhật trong
chính file đó) — không thuộc phạm vi tài liệu Grasshopper nên tách riêng khỏi 4 skill bên dưới.
...
**Ưu tiên hàng đầu xuyên suốt mọi nghiên cứu: cách tạo PATTERN bằng Grasshopper** (hoạ tiết/mẫu lặp
lại có kiểm soát tham số — mặt đứng, lưới hoạ tiết, mảng module, tessellation, attractor-driven
pattern...). Đây là mục tiêu học tập chính của user, không chỉ là 1 lựa chọn khi mục "Ứng dụng"/
"Ví dụ" của skill `nghien-cuu-grasshopper` tình cờ hợp — luôn chủ động tìm và nêu góc nhìn "component
này dùng thế nào để tạo pattern" trước tiên khi nghiên cứu bất kỳ component nào, chỉ chuyển sang góc
nhìn khác (Architecture, Structural, Analysis...) khi component thực sự không gắn được với pattern
một cách tự nhiên/trung thực.
